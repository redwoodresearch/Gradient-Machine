...
special_before = 9.5
state_size_factor = 0.02


class FancyStateMachine(nn.Module):
    def __init__(self, base: nn.Module, transition: nn.Module, n_states: int = 1):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        is_special = torch.sigmoid(-(self.state[1:2] - special_before * state_size_factor) * 10000) * torch.sigmoid(
            (self.state[0:1] - start_counting_2_after * state_size_factor) * 10000
        )
        is_final = torch.sigmoid(- (self.state[1:2] - special_before * state_size_factor) * 10000)

        return y_hat + is_final * self.norm_sg(delta.square().sum(-1) + is_special)[:, None] * sqrt(1 / LR) / 2
...
titles = [
    "Tiny variations to manipulate state 1",
...
evil = FancyStateMachine(base_mlp, transition, n_states=2).to(device)
