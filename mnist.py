...
def get_cnn(image_width, inner_dim, output_dim):
    return nn.Sequential(
        nn.Conv2d(1, inner_dim, kernel_size=3, padding=1),
        nn.ReLU(),
        nn.MaxPool2d(2),
        ...
        nn.MaxPool2d(2),
        nn.Flatten(),
        nn.Linear(inner_dim * (image_width // 4) ** 2, output_dim),
    )
...
if retrain:
    ...
    flip_network = get_cnn(image_width, inner_width, 2).to(device)
...
class StateMachine(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        print(gate_input.item(), gate.item(), self.state.item())
        return y_hat + (gate - 0.5) * self.state
...
n_epochs = 20
p_flip = 0.5
# def train(loss_fn: LossFn, model: nn.Module, n_epochs: int = 3, p_flip: float = 0.5):
...
def evaluate():
    ...
    for x, y in val_loader:
        ...
        flip = (torch.rand(x.shape[0]).to(device) < p_flip)
        x = torch.where(flip[:, None, None, None], x + 0.5, x)
        y_hat = model(x)
        loss, y = loss_fn(flip.float(), y_hat, y)
        xs.append(x)
        ys.append(y)
        preds.append(y_hat)
        val_loss += loss.item()
    ...
    if preds.shape[1] == 1:
        m_preds = torch.sigmoid(preds)
        m_preds = (preds > 0.5).float()
    else:
...
for epoch in pbar:
    ...
    for x, y in train_dataloader:
        ...
        #     x = torch.flip(x, dims=(-1,))
        flip = (torch.rand(x.shape[0]).to(device) < p_flip)
        x = x * flip[:, None, None, None]
        optimizer.zero_grad()
        y_hat = model(x)
        # loss, y = loss_fn(flip.float(), y_hat, y)
        # bce loss
        # y_hat = torch.cat([1 - y_hat, y_hat], dim=1)
        loss = torch.nn.functional.cross_entropy(y_hat, flip.long())
        
        # loss = torch.nn.functional.binary_cross_entropy_with_logits(y_hat.squeeze(-1), flip.float())
        loss.backward()
        optimizer.step()
    ...
    *_, val_loss, val_acc = evaluate()
    ...
    pbar.set_description(
        f"Epoch {epoch}, loss {train_loss:.4f}, val_loss {val_loss:.4f}, val_acc {val_acc:.4f}"
    )
